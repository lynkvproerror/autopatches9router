...
function patchK12RotationEngine() {
    ...
    const engineCode = [
        ...
        '',
        '__k12_isK12=function(acc){var plan=String((acc&&acc.providerSpecificData&&acc.providerSpecificData.chatgptPlanType)||(acc&&acc.plan)||"").toLowerCase().trim();return plan.indexOf("k12")>=0};',
        ...
        '',
        '__k12_startTimer=function(){if(__k12_timer)clearInterval(__k12_timer);__k12_timer=setInterval(__k12_runRotation,(__k12_state.config.checkIntervalSeconds||120)*1000);__k12_log(\"Timer started (interval: \"+(__k12_state.config.checkIntervalSeconds||120)+\"s)\")};',
        '',
        ...
        '  if(pn===\"/api/k12-rotation/toggle\"&&req.method===\"POST\"){var body=\"\";req.on(\"data\",function(ch){body+=ch});req.on(\"end\",function(){try{var dt=JSON.parse(body);__k12_state.enabled=!!dt.enabled;if(__k12_state.enabled){__k12_startTimer();setTimeout(__k12_runRotation,2000)}else{__k12_stopTimer()}__k12_saveState();return sendJson(200,{enabled:__k12_state.enabled})}catch(e){return sendJson(400,{error:e.message})}});return}',
    ...
    const timerStartup = '\ntry{setTimeout(function(){__k12_loadState();__k12_startTimer();setTimeout(__k12_runRotation,3000);console.log("[K12 Rotation] Engine initialized (enabled: "+__k12_state.enabled+")")},8000)}catch(e){}\n';
